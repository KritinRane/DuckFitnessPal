...
function deriveTargets(f: FormState) {
  if (!f.weightLbs || !f.heightFt || !f.age || !f.sex || !f.activityLevel || !f.goal) return null;
  const tdee = calculateTDEE(
    Number(f.weightLbs), Number(f.heightFt), Number(f.heightIn),
    Number(f.age), f.sex as Sex, f.activityLevel as ActivityLevel
  );
  const calories = calculateGoalCalories(tdee, f.goal as Goal, f.cutRate, f.bulkRate);
  const macros = calculateMacros(calories, Number(f.weightLbs), f.goal as Goal);
  return { tdee, calories, ...macros };
}

const inputCls = "w-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 placeholder:text-gray-300 dark:placeholder:text-gray-600";
  ...
  async function handleApply() {
    ...
    try {
      await fetch("/api/profile", {
        ...
        body: JSON.stringify({
          weightLbs: Number(form.weightLbs), heightFt: Number(form.heightFt),
          heightIn: Number(form.heightIn), age: Number(form.age),
          sex: form.sex, activityLevel: form.activityLevel,
          goal: form.goal, cutRate: form.cutRate, bulkRate: form.bulkRate,
        }),
      });
      await fetch("/api/goals", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ calories: targets.calories, protein: targets.protein, carbs: targets.carbs, fat: targets.fat }),
      });
      setSaved(true);
  ...
  return (
    <main className="max-w-md mx-auto px-5 pt-10 pb-10 space-y-8">
      ...
      <section className="space-y-4">
        <h2 className="text-xs font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Body Stats</h2>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Weight (lbs)</label>
            <input type="number" min={50} max={500} placeholder="170" value={form.weightLbs}
              onChange={(e) => set("weightLbs", e.target.value)} className={inputCls} />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Age</label>
            <input type="number" min={13} max={100} placeholder="20" value={form.age}
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              <input type="number" min={3} max={8} placeholder="5" value={form.heightFt}
                onChange={(e) => set("heightFt", e.target.value)}
                className={`${inputCls} pr-8`} />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400">ft</span>
            </div>
            <div className="relative flex-1">
              <input type="number" min={0} max={11} placeholder="10" value={form.heightIn}
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Sex</label>
      ...
      <section className="space-y-3">
        <h2 className="text-xs font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Activity Level</h2>
        <div className="space-y-2">
          {(Object.keys(ACTIVITY_LABELS) as ActivityLevel[]).map((level) => (
            <button key={level} onClick={() => set("activityLevel", level)}
              ...
              <span className={`text-sm font-medium ${form.activityLevel === level ? "text-red-700 dark:text-red-400" : "text-gray-700 dark:text-gray-300"}`}>
      ...
      {/* Goal */}
      <section className="space-y-3">
        <h2 className="text-xs font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Goal</h2>
        <div className="grid grid-cols-3 gap-2">
          {(
            ...
            <button key={key} onClick={() => set("goal", key)}
              ...
              <span className={`text-sm font-semibold ${form.goal === key ? "text-red-700 dark:text-red-400" : "text-gray-700 dark:text-gray-300"}`}>
                {label}
              </span>
          ...
          <div className="space-y-2 pt-1">
            ...
            <div className="grid grid-cols-2 gap-2">
              {CUT_RATES.map(({ key, label, deficit, lossPerWeek }) => (
                <button key={key} onClick={() => set("cutRate", key)}
                  className={`flex flex-col items-start px-3 py-2.5 rounded-xl border text-left transition-colors ${
                    form.cutRate === key
                      ? "border-red-600 bg-red-50 dark:bg-red-900/20"
                      : "border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 hover:border-gray-200 dark:hover:border-gray-700"
                  }`}>
                  <span className={`text-sm font-medium ${form.cutRate === key ? "text-red-700 dark:text-red-400" : "text-gray-700 dark:text-gray-300"}`}>
                    {label}
                  </span>
                  <span className="text-[10px] text-gray-400 mt-0.5">-{deficit} kcal · {lossPerWeek}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2 pt-1">
            ...
            <div className="grid grid-cols-2 gap-2">
              {BULK_RATES.map(({ key, label, surplus, gainPerWeek }) => (
                <button key={key} onClick={() => set("bulkRate", key)}
                  className={`flex flex-col items-start px-3 py-2.5 rounded-xl border text-left transition-colors ${
                    form.bulkRate === key
                      ? "border-red-600 bg-red-50 dark:bg-red-900/20"
                      : "border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 hover:border-gray-200 dark:hover:border-gray-700"
                  }`}>
                  <span className={`text-sm font-medium ${form.bulkRate === key ? "text-red-700 dark:text-red-400" : "text-gray-700 dark:text-gray-300"}`}>
                    {label}
                  </span>
                  <span className="text-[10px] text-gray-400 mt-0.5">+{surplus} kcal · {gainPerWeek}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      {/* Preview */}
      {targets && (
        <section className="bg-gray-900 dark:bg-gray-800 rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3 pt-1 border-t border-gray-800 dark:border-gray-700">
            {[
