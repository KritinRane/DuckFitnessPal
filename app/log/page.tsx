...
export default function LogPage() {
  ...
  return (
    <main className="flex-1 max-w-md mx-auto w-full px-5 pt-10 pb-8 space-y-6">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Food Log</h1>
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          ...
          <div key={label} className="bg-gray-50 dark:bg-gray-900 rounded-xl p-3 text-center">
            <p className="text-lg font-bold text-gray-900 dark:text-white tabular-nums leading-none">{value}</p>
            <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mt-1">{unit}</p>
            <p className="text-[9px] text-gray-300 dark:text-gray-600 mt-0.5">{label}</p>
          </div>
        ))}
      </div>
      ...
      {loading ? (
        <div className="space-y-2">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="animate-pulse bg-gray-50 dark:bg-gray-900 rounded-xl h-16" />
          ))}
        </div>
      ) : entries.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-xs text-gray-300 dark:text-gray-600 mt-1">Go to Menu and tap + to add meals</p>
        </div>
      ) : (
